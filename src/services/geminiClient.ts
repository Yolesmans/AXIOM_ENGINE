...
export async function testGemini(): Promise<string> {
  if (GEMINI_MOCK_ENABLED) return 'OK';
...
export async function callGemini(params: {
  ...
  if (AXIOM_TEST_MODE) return 'MOCK_RESPONSE_TEST';
...
export type CallGeminiStreamOpts = {
  messages: Array<{ role: string; content: string }>;
  model?: string;
  temperature?: number;
  max_tokens?: number;
...
export type CallOpenAIStreamOpts = CallGeminiStreamOpts;
