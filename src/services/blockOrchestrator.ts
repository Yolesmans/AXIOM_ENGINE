import type { AxiomCandidate, NormalizedWork, NormalizedCharacter } from '../types/candidate.js';
...
function normalize2A1Response(raw: string): 'SERIE' | 'FILM' | null {
  ...
  if (s === 'a' || s === 'a.' || s === 'série' || s === 'serie' || s === 'une série' || s === 'une serie' || s.startsWith('a.') || s.startsWith('a ')) return 'SERIE';
  if (s === 'b' || s === 'b.' || s === 'film' || s === 'un film' || s.startsWith('b.') || s.startsWith('b ')) return 'FILM';
  // Détection souple
  if (s.includes('série') || s.includes('serie')) return 'SERIE';
...
export interface OrchestratorResult {
  ...
  /** Miroir 2B (affiché puis première question bloc 3 dans la même réponse) */
  mirror?: string;
  /** Première question bloc 3 (enchaînement auto après miroir 2B) */
  nextQuestion?: string;
}
  ...
  async handleMessage(
    ...
    if (currentBlock === 2 && (currentStep === BLOC_02 || currentStep === '')) {
      const blockStates = candidate.session.blockStates ?? {
        '2A': { status: 'NOT_STARTED' as const },
        '2B': { status: 'NOT_STARTED' as const, currentQuestionIndex: 0 },
      };
...
Génère 3 à 5 questions maximum pour le BLOC 1.`,
  ...
  private async handleBlock2A(
    ...
    const blockNumber = 2;
    const candidateId = candidate.candidateId;
    let currentCandidate: AxiomCandidate | undefined = candidateStore.get(candidateId) ?? await candidateStore.getAsync(candidateId);
    if (!currentCandidate) throw new Error(`Candidate ${candidateId} not found`);
    ...
    if (!userMessage) {
      ...
      if (lastQuestion) return { response: normalizeSingleResponse(lastQuestion), step: BLOC_02, expectsAnswer: true, autoContinue: false };
      return { response: normalizeSingleResponse(STATIC_QUESTION_2A1), step: BLOC_02, expectsAnswer: true, autoContinue: false };
    }
  ...
  private async generateQuestion2A1(_candidate: AxiomCandidate, _retryCount: number = 0): Promise<string> {
    // Formulation EXACTE du SUPER-PROMPT V8 — 0 token LLM, formulation garantie
    return STATIC_QUESTION_2A1;
  }

  private async generateQuestion2A2(_candidate: AxiomCandidate, mediumAnswer: string): Promise<string> {
    // Formulation EXACTE du SUPER-PROMPT V8 — 0 token LLM, formulation garantie
  ...
  private async generateQuestion2A3(_candidate: AxiomCandidate, _answers: Record<number, string>, _retryCount: number = 0): Promise<string> {
    // Formulation EXACTE du SUPER-PROMPT V8 — 0 token LLM, formulation garantie
  ...
  private async handleBlock2B(
    ...
    if (!block2A?.medium || !block2A?.preference || !block2A?.coreWork) {
      return safeReturnMessage(
        ...
        'BLOC 2A answers missing (block2A)'
      );
    }
    const mediumAnswer = block2A.medium;
    ...
    if (blockStates?.['2B']?.status !== 'IN_PROGRESS' && userMessage) {
      return safeReturnMessage(
        "L'état du bloc 2B est incohérent. Recharge la page.",
        'BLOC 2B status not IN_PROGRESS'
      );
    }
    ...
    const works: string[] =
      normalizedWorks && normalizedWorks.length >= 1
        ? normalizedWorks.map((w) => w.canonicalTitle)
        : this.parseWorks(preferencesAnswer);

    if (works.length === 0) {
      return safeReturnMessage(
        "Tu n'as pas indiqué d'œuvre dans ta réponse précédente. Peux-tu me donner au moins une série ou un film qui te parle (ex. : Breaking Bad, Dark, Suits) ?",
        'No works for 2B'
      );
    }
    ...
    if (userMessage) {
      ...
      if (looksLikeChoiceAE) {
        ...
        if (normChars.characters && normChars.characters.length >= 1) {
          ...
          const newMeta: Block2BQuestionMeta[] = [];
          // PERF : génération traits parallèle (Promise.all) — était séquentielle
          const traitResults = await Promise.all(
            normChars.characters.map(ch => this.generateTraitsForCharacterLLM(work, ch.canonicalName))
          );
  ...
  private async normalizeWorksLLM(raw: string): Promise<{ works?: NormalizedWork[]; needsClarification?: boolean; message?: string }> {
    const completion = await callOpenAI({
      messages: [
        {
          role: 'system',
...
Sinon : {"works":[{"canonicalTitle":"Titre officiel","type":"series" ou "film","confidence":0.95},...]} (1 à 3 œuvres).`,
    ...
    try {
      const parsed = JSON.parse(completion.replace(/^```\w*\n?|\n?```$/g, '').trim()) as { works?: NormalizedWork[]; needsClarification?: boolean; message?: string };
...
Ordre : motif #1, personnages #1, motif #2, personnages #2, motif #3, personnages #3.`;
    ...
    const tryParse = (): { questions: string[]; meta: Block2BQuestionMeta[] } | null => {
      try {
...
Œuvres : 1=${w0}, 2=${w1}, 3=${w2}. Ordre : motif1, personnages1, motif2, personnages2, motif3, personnages3. Aucun texte avant ou après le tableau.`;
    ...
    const generateMotifWithRetry = async (workTitle: string): Promise<string> => {
    ...
    const motifQuestions = await Promise.all(
      [works[2] ?? w2, works[1] ?? w1, works[0] ?? w0].map(workTitle => generateMotifWithRetry(workTitle))
    );
  ...
  private async generateOneMotifQuestionForWork(work: string): Promise<string> {
...
Génère maintenant les 5 propositions SPÉCIFIQUES à « ${title} ».`;
    ...
    try {
      ...
      if (this.hasMotifAE(text) && this.hasMinWordCountInOptions(text)) return text;
    } catch {
      /* ignore */
    }
...
Chaque proposition doit être UNIQUE à ${title}, évocatrice, en français. INTERDIT de répondre "Le thème central", "Les personnages", "L'intrigue", "L'univers" ou tout terme générique.`,
      ...
      if (this.hasMotifAE(text2)) return text2;
    } catch {
      /* ignore */
    }
  ...
  private hasMinWordCountInOptions(text: string): boolean {
  ...
  private async generateTraitsForCharacterLLM(work: string, character: string, _context?: string): Promise<{ question: string; options: string[] }> {
    ...
    const hasEnoughWords = (opt: string): boolean => opt.trim().split(/\s+/).length >= 5;
    const parseResponse = (raw: string): { question: string; options: string[] } | null => {
      try {
        ...
        const parsed = JSON.parse(cleaned.slice(start, end + 1)) as { question?: string; options?: string[] };
        ...
        if (!rawOpts || !rawOpts.every(hasEnoughWords)) return null;
...
[Micro-récap Œuvre #1]`
   ...
   */
  private async getNextQuestion2BContentOnly(candidateId: string, blockNumber: number): Promise<OrchestratorResult> {
   ...
   */
  private async serveNextQuestion2B(candidateId: string, blockNumber: number): Promise<OrchestratorResult> {
    const candidate = candidateStore.get(candidateId);
    if (!candidate) throw new Error(`Candidate ${candidateId} not found`);

    const queue = candidate.blockQueues?.[blockNumber];
    if (!queue || queue.questions.length === 0) throw new Error(`Queue for block ${blockNumber} is empty`);
...
Génère le miroir projectif en suivant EXACTEMENT le format en 4 sections (1️⃣ 2️⃣ 3️⃣ 4️⃣).`;
      ...
      if (occupationTimer) { clearTimeout(occupationTimer); occupationTimer = null; }
      ...
      if (!mentorText) throw new Error('Empty mirror text from GPT-4.1');
