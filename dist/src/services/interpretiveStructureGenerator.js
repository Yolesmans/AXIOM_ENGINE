 ...
 */
export async function generateInterpretiveStructure(userAnswers, blockType, additionalContext) {
...
Produis UNIQUEMENT un JSON valide avec TOUS les champs remplis de manière PRÉCISE et SPÉCIFIQUE, sans texte additionnel.`
            ...
            let structure;
            try {
                structure = JSON.parse(content.trim());
            }
            catch (parseError) {
 ...
 */
export async function generateInterpretiveStructureBlock1(userAnswers) {
    return generateInterpretiveStructure(userAnswers, 'block1');
}
/**
 * Retourne le contexte spécifique au bloc (contexte ≠ profondeur)
 */
function getBlockContext(blockType, additionalContext) {
    switch (blockType) {
        case 'block1':
...
function validateStructure(structure) {
    ...
    if (hypotheseCentrale.length > 0) {
        ...
        const vagueCount = vagueWords.filter(word => hypotheseCentrale.toLowerCase().includes(word)).length;
        if (vagueCount > 2) {
