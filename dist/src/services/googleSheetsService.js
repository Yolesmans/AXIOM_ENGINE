...
export function getAxiomLink(tenantId, posteId) {
    const base = process.env.AXIOM_PUBLIC_URL;
    if (!base)
        throw new Error('AXIOM_PUBLIC_URL missing');
    return `${base}/start?tenant=${tenantId}&poste=${posteId}`;
}
export function candidateToLiveTrackingRow(candidate) {
    const state = candidate.session.completedAt
        ? 'completed'
...
class GoogleSheetsLiveTrackingService {
    ...
    async ensureSheetExists(spreadsheetId, sheetName, tenantId, posteId) {
    ...
    async ensureHeaderRow(spreadsheetId, sheetName, tenantId, posteId) {
        if (!this.sheets) {
            throw new Error('Google Sheets not initialized');
        }
        try {
    ...
    async applyConditionalFormatting(spreadsheetId, sheetName) {
        if (!this.sheets) {
            throw new Error('Google Sheets not initialized');
        }
        try {
