 ...
 */
function extractPropositions(text) {
    return text
        .split(/\n/)
 ...
 */
export function validateTraitsSpecificity(traitsWork1, traitsWork2, traitsWork3) {
 ...
 */
export function validateMotifsSpecificity(motifWork1, motifWork2, motifWork3) {
 ...
 * - Longueur : 4-6 lignes
 */
export function validateSynthesis2B(content) {
    ...
    if (!hasMotifs || !hasPersonnages || !hasTraits) {
        ...
        if (!hasMotifs)
            missingElements.push('motifs');
        if (!hasPersonnages)
            missingElements.push('personnages');
        if (!hasTraits)
            missingElements.push('traits');
        return {
            valid: false,
 ...
 */
export function validateQuestion2A1(content) {
 ...
 */
export function validateQuestion2A3(content) {
    if (!content || typeof content !== 'string') {
        return {
            valid: false,
    ...
    if (!asksForOne || !asksForWork) {
        const missing = [];
        if (!asksForOne)
            missing.push('demande d\'une œuvre UNIQUE');
        if (!asksForWork)
            missing.push('mention d\'œuvre/série/film');
        return {
            valid: false,
